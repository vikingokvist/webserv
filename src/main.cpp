...
int main(int argc, char **argv)
{
	...
	try
	{
		...
		// Setup all servers
		// multiples puertos, multiples hosts pueden ser añadidos
			...
			for (size_t i = 0; i < pollfds.size(); i++) {
				...
				if (pollfds[i].revents & POLLIN) {
					...
					if (_connection.setConnection(servers[i])) {
						...
						ssize_t best_match = _connection.getBestMatch(servers[i], _connection.getHeader("Path"));
						if (best_match != -1 && _connection.receiveRequest(best_match)) {
